import pandas as pd
# """提供待清洗csv的名字（字符串）。然后将某列设置为“不区分方向”，然后把清除掉重复行"""
# def clean_csv(cleaning_file_name,colum_name):
#     df = pd.read_csv(cleaning_file_name)
#     df.loc[1:, colum_name] = '不区分方向'
#     df.drop_duplicates(inplace=True)
#     df.to_csv('new_file.csv', index=False)
def copy_paste_excel_columns(file_name,n,savepath):
    if n == 1:
    # 读取表格1中专业模板列数据
        ...
        # 存储表格2
        savename =  savepath + '\专业模板' + '.xlsx'
        df2.to_excel(savename, index=False)
    if n == 2:
    # 读取表格1中方向模板列数据
        df1 = pd.read_csv(file_name, usecols=[0,2,3,4])
        # 新建表格2，粘贴到表格2中A列和B列内容
        df2 = pd.DataFrame({'学校名称': df1.iloc[:, 0], '学院名称': df1.iloc[:, 1],'专业名称': df1.iloc[:, 2],'方向名称': df1.iloc[:, 3]})
        # df2 = pd.DataFrame({'学校名称': df1.iloc[:, 0], '学院名称': df1.iloc[:, 1],'专业名称': df1.iloc[:, 2]},'方向名称': df1.iloc[:, 3]})
        # 存储表格2
        savename = savepath + '\方向模板' + '.xlsx'
        df2.to_excel(savename, index=False) 
    if n == 3:
    # 读取表格1中学院模板列数据
        df1 = pd.read_csv(file_name, usecols=[0,2])
        # 新建表格2，粘贴到表格2中A列和B列内容
        df2 = pd.DataFrame({'学校名称': df1.iloc[:, 0], '学院名称': df1.iloc[:, 1]})
        # 存储表格2
        savename = savepath + '\学院模板' + '.xlsx'
        df2.to_excel(savename, index=False)
    if n == 4:
    # 读取表格1中学校模板列数据
        df1 = pd.read_csv(file_name, usecols=[0])
        # 新建表格2，粘贴到表格2中A列和B列内容
        df2 = pd.DataFrame({'学校名称': df1.iloc[:, 0]})
        # 存储表格2
        savename = savepath +  '\学校模板' + '.xlsx'
        df2.to_excel(savename, index=False)